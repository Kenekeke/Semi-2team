...
<script>
    $(function () {
        ...
        $.ajax({
        	async:false,
        	url:"<%=request.getContextPath()%>/house/list_villatwo.do",
        	type: "POST",
        	data: {
      			...
      			$.each(markers, function(index, marker){
      				if(bounds.contain(marker.getPosition())){
      					...
      					$.ajax({
        					...
        					url : "<%=request.getContextPath()%>/house/filter_villatwo.do",
        					type : "POST",
        					data : {
        						villatwo_no : marker.no
        					},
        					success:function(resp){
        						...
        						if(resp[0].monthly==0){
       				            ...
       				            var area_floor = resp[0].floor+" ㆍ "+resp[0].area+"㎡";
       				         	$(template).find(".filterImg").children().attr("src", "<%=request.getContextPath()%>/member/photodownload.do?house_no="+resp[0].house_no+"&photo_no="+resp[0].photo_no).appendTo($(".list").children().last());
        						...
        						$(".list").children().last().click(function(){
        							...
        							$.each(resp, function(index, info){
        								...
        								$(template_img).find(".img-img").attr("src", "<%=request.getContextPath()%>/member/photodownload.do?house_no="+info.house_no+"&photo_no="+info.photo_no).appendTo($(".image-box").children().last());
...
<aside class="filtering">
	...
	<div class="ListAndFilter">
		...
		<form action="villatwo.jsp" method="post">
			<div class="active">
				<input type="hidden" name="filter" value="a"> <input
